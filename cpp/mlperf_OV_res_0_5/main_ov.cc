...
int main(int argc, char **argv) {
    ...
    size_t total_sample_count = 500, perf_sample_count = 200, nstreams = 1,
    ...
    mp.insert(std::pair<std::string, int>("--multi_device_streams", 17)); // MULTI device streams. e.g --multi_device_streams CPU:4,GPU:8
    ...
    sut = std::unique_ptr < Sut
            > (new Sut(settings, nireq, nstreams, nthreads, model, batch_size,
                    dataset, workload, device, multi_device_streams));
    // Init Dataset
